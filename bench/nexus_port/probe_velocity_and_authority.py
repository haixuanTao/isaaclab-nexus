...
PART = sys.argv[2] if len(sys.argv) > 2 else "both"; BACKEND = os.environ.get("NEXUS_BACKEND", "nexus")
...
def make(gravity, N):
    ...
    env = gym.make(TASK, cfg=cfg).unwrapped; env.reset(); return env
...
if PART in ("1", "both"):
  ...
  prev = robot.data.joint_pos.torch[0, j].item(); print(f"[{BACKEND} gain x{GAIN}] step   q        v_reported   dq/dt      torque")
  for i in range(12):
      robot.set_joint_position_target(tgt); robot.write_data_to_sim(); (NexusManager.step() if BACKEND == "nexus" else env.sim.step(render=False)); robot.update(dt)
...
if PART in ("2", "both"):
  ...
  tq = robot.data.applied_torque.torch.abs(); print(f"[{BACKEND}] gain x{GAIN}: " + " | ".join(out) + f" | |torque| median {float(tq.median()):.1f} max {float(tq.max()):.1f} N·m")
  env.close(); app.close()
