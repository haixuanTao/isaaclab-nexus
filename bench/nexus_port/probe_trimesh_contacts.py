...
pose = robot.data.default_root_state.torch.clone(); pose[:, :3] = torch.tensor([0, 0, 0.5], device="cuda:0"); pose[:, 3:7] = quats[0]
...
low = []
import numpy as _np
lay = st.rigid_contact_layout(); print("contact layout:", lay)
def _t(ca): return torch.as_tensor(ca) if not hasattr(ca, "__cuda_array_interface__") else torch.utils.dlpack.from_dlpack(ca) if hasattr(ca, "__dlpack__") else torch.as_tensor(_np.asarray(ca))
for i in range(800):
    ...
    if i in (40, 55, 60, 65, 70, 80, 100, 200):
        try:
            cl = torch.as_tensor(st.contacts_len_cuda(), device="cuda:0"); rc = torch.as_tensor(st.rigid_contacts_cuda(), device="cuda:0")
            n0 = int(cl[0]); feet_z = robot.data.body_link_pos_w.torch[0, robot.find_bodies(".*ankle_roll_link")[0], 2].cpu().numpy()
            print(f"step {i} t={(i+1)*DT:.3f}s env0 contacts={n0} (all envs: min {int(cl.min())} max {int(cl.max())}) feet z {feet_z.round(3)} lowest body {low[-1][0]:+.3f}")
            print("   raw contacts (env 0, first 6 rows):", rc.reshape(-1, rc.shape[-1])[:6].cpu().numpy().round(3) if rc.dim() > 1 else rc[:48].cpu().numpy().round(3))
        except Exception as e: print("contact readback failed:", repr(e)[:200])
