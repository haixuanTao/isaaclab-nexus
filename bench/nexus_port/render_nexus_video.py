...
for k in range(K):                                                                  # each env on ITS OWN tile
    vk, fk = (z[f"terrain_v{k}"], z[f"terrain_f{k}"]) if f"terrain_v{k}" in z else (z["terrain_v"], z["terrain_f"])
    tm = trimesh.Trimesh(vk, fk, process=False)
    if tm.face_normals[:, 2].mean() < 0: tm.invert()                              # MuJoCo back-face culls; normals must point up
    tm.export(f"{D}/terrain_env{k}.stl")
xml = open(SRC).read()
xml = xml.replace('meshdir="meshes"', f'meshdir="{os.path.dirname(SRC)}/meshes"')
xml = xml.replace("</asset>", "".join(f'<mesh name="terrain_tile{k}" file="{D}/terrain_env{k}.stl" inertia="shell"/>' for k in range(K)) + '<texture type="skybox" builtin="gradient" rgb1=".55 .7 .9" rgb2=".95 .97 1" width="256" height="256"/></asset>', 1)
...
xml = xml.replace("<worldbody>", '<worldbody><light pos="0 0 6" dir="0 0 -1" diffuse=".9 .9 .9"/><light pos="3 -3 4" dir="-.5 .5 -1" diffuse=".5 .5 .5"/>'
                  + "".join(f'<geom name="tile{k}" type="mesh" mesh="terrain_tile{k}" rgba=".58 .55 .5 1" contype="0" conaffinity="0" group="{2+k}"/>' for k in range(K)), 1)
...
opt = mujoco.MjvOption(); opt.geomgroup[:] = 0; opt.geomgroup[1] = 1; opt.geomgroup[0] = 1     # visual geoms; tile k toggled per env below
...
for t in range(T):
    ...
    for k in range(K):
        ...
        for kk in range(K): opt.geomgroup[2 + kk] = 1 if kk == k else 0
