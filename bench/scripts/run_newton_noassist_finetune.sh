#!/bin/bash
# Fine-tune a checkpoint with the lift harness removed (AGILE_NO_ASSIST=1), same physics stack.
# Usage: run_newton_noassist_finetune.sh <load_run> <checkpoint.pt> <total_iters> <num_envs>
set -u
LOAD_RUN="$1"; CKPT="$2"; ITERS="${3:-10000}"; N="${4:-4096}"
RES=/workspace/bench/results_newton
TAG="newton-noassist-ft_from${CKPT%.pt}_train${ITERS}_n${N}_$(date +%H%M%S)"
echo "=== [$(date +%T)] $TAG ==="
