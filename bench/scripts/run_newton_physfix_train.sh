#!/bin/bash
# Newton training with the physics fixes on: flat ground, implicit actuators,
# implicitfast, DC-motor torque-speed envelope in-solver, MuJoCo-native joint
# limits, velocity clamp OFF.  Usage: run_newton_physfix_train.sh <iters> <num_envs>
set -u
ITERS="${1:-1000}"; N="${2:-4096}"
RES=/workspace/bench/results_newton
TAG="newton-physfix_train${ITERS}_n${N}_$(date +%H%M%S)"
echo "=== [$(date +%T)] $TAG ==="
...
( env HOME=/root OMNI_KIT_ACCEPT_EULA=YES \
    ...
    AGILE_NEWTON_DC_ENVELOPE=1 AGILE_NEWTON_LIMIT_SOLREF=0.02,1 AGILE_NEWTON_VEL_CLAMP=0 AGILE_NAN_WATCHDOG=1 \
